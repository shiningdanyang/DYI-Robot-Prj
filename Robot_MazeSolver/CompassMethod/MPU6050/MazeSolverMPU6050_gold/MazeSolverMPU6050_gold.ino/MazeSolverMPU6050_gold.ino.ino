char path[30] = {"SLSSSLRD"};
//char path[30]={"LLRSSRR"};
int readLength = 0;
int changeStatusDelay = 2000;
int bias = 0; //turn angle too sharp -> increase bias
int leftThreshold = 35;
int rightThreshold = 35;
int calibSpeed = 20;//12
int leapTime = 500;
int leftSide = 8;
int rightSide = 14;
int deltaSpeedSkip = 10;
#include <AFMotor.h>
...
SimpleKalmanFilter filterAngle(2, 2, 0.04);
...
int setpoint = 0;
int initialSpeedLeft = 235;
int initialSpeedRight = 230;//75
int turnSpeed;
...
float KpDis = 7,
      KiDis = 0,
      KdDis = 200;
int errorDis, PDis, IDis, DDis, PID_valueDis;
float previous_errorDis;

float Kp = 15,
      Ki = 0,
      Kd = 300;
int error, P, I, D, PID_value;
...
int degree()
{
  ...
  //  Serial.println(F(""));
  angle_z = filterAngle.updateEstimate(angle_z);
  //Serial.println(angle_z);
  return ((int)angle_z);
}

void calculatePIDDistance()
{
  errorDis =  disLeft - setpointDis;
  PDis = errorDis;
  IDis = IDis + errorDis;
  DDis = errorDis - previous_errorDis;
  PID_valueDis = (KpDis * PDis) + (KiDis * IDis) + (KdDis * DDis);
  //  Serial.println(error);
  previous_errorDis = errorDis;
}

void motorControlDistance()
{
  // Calculating the effective motor speed:
  int leftSpeedDis = initialSpeedLeft - PID_valueDis;
  int rightSpeedDis = initialSpeedRight + PID_valueDis;

  leftSpeedDis = constrain(leftSpeedDis, 0, 255);
  rightSpeedDis = constrain(rightSpeedDis, 0, 255);
  motorLeft.run(FORWARD);
  motorRight.run(FORWARD);
  motorLeft.setSpeed(leftSpeedDis);
  motorRight.setSpeed(rightSpeedDis);
...
{
  // Calculating the effective motor speed:
  int leftSpeed = initialSpeedLeft - PID_value;
  int rightSpeed = initialSpeedRight + PID_value;
...
int distance(int _position)
{
  digitalWrite(trig[_position], LOW);  // tắt chân trig
  delayMicroseconds(2);
  digitalWrite(trig[_position], HIGH);  // phát xung từ chân trig
  delayMicroseconds(10);  // xung có độ dài 10 microSeconds (độ dài xung tiêu chuẩn của SR05)
  digitalWrite(trig[_position], LOW);  // tắt chân trig
  int duration = pulseIn(echo[_position], HIGH);
  int _distance = int(duration / 2 / 29.412);
...
void runStraight()
{
  if (disLeft < leftSide)
  {
    motorLeft.run(FORWARD);
    motorRight.run(FORWARD);
    motorLeft.setSpeed(initialSpeedLeft + calibSpeed);
    motorRight.setSpeed(initialSpeedRight - calibSpeed);
    delayMicroseconds(500);
    return 0;
//    calculatePIDDistance();
//    motorControlDistance();
  }
  if (disLeft > rightSide)
  {
    motorLeft.run(FORWARD);
    motorRight.run(FORWARD);
    motorLeft.setSpeed(initialSpeedLeft - calibSpeed);
...
void runSkip(int _speed, int _time)
{
  ...
  motorLeft.run(FORWARD);
  motorRight.run(FORWARD);
  motorLeft.setSpeed(_speed + deltaSpeedSkip);
...
void runSkip2()
{
  while ((distance(left) > leftThreshold) || (distance(right) > rightThreshold))
  {
    calculatePID();
    motorControl();
  }
...
void turnLeft()
{
  ...
  delay(10);
  setpoint = degree();
  setpoint = setpoint - (90 - bias);
  while (degree() > setpoint)
  {
    error2 = (degree() - setpoint);
    calculateRotaPID();
    rotaLeft(turnSpeed);
  ...
  stop();
  delay(30);
  //  while ((disLeft > leftThreshold) || (disRight > rightThreshold))
  //  {
  //    runStraight();
...
void replay()
{
  disLeft = distance(left);
  disRight = distance(right);
  if (disLeft < leftThreshold && disRight < rightThreshold)  //nếu không có ngã rẽ
  {
    runStraight();
  }
  else  //nếu phát hiện có ngã rẽ
  {
    if (path[readLength] == 'D')
    {
      stop();
      delay(changeStatusDelay);
      motorLeft.run(FORWARD);
      motorRight.run(FORWARD);
...
void loop()
{
  ...
  //  if (distance(left) < leftThreshold)
  //  {
  //    runStraight();
